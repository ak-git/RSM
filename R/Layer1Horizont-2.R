...
plot(hToL, y1, type = "l", lwd = 2, log = "xy", ylim = c(rho1, rho2),
     ...
     ylab = expression(bold(frac(rho[apparent], rho[1])))
)
...
abline(v = hToL_Horizont(deltaL = deltaL), h = 6 * deltaL + 1, lty = 3, lwd = 2)
abline(v = hToL_Horizont(deltaL = deltaL, k = k), h = 6 * deltaL + 1, lty = 3, lwd = 2)
text(x = 0.1, y = 6 * deltaL + 1, expression(delta ~ rho[1] == frac(1 + over(s, L), over(s, L) %.% (1 - over(s, L))) %.% delta ~ L == 6 %.% delta ~ L == "2 %"),
     pos = 3, srt = 0)
...
abline(v = hToL_HorizontMin(deltaL = deltaL, k = k), h = rho2 * (1 - 6 * deltaL), lty = 3, lwd = 2)
